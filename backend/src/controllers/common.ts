...
export const createErrorResponse = (reason: string, field: string | null = null): ApiResponse<object> => {
  return {
    status: 'failure',
    error: reason,
    data: {field: field}
...
export const handleMissingField = (missingField: string): ApiResponse<object> => {
  return createErrorResponse("Required field is missing.", missingField);
}

export const getRequiredField = (data: { [x: string]: any; }, field: string) => {
