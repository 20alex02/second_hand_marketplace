...
const generateAccessToken = async (uuid: string, secretKey: string) => {
  return jwt.sign({username: uuid}, secretKey as Secret, { expiresIn: expiresIn });
}
...
export const hashPassword = (password: string, salt: string | null = null): {hashedPassword: string, salt: string} => {
  if (salt === null) {
